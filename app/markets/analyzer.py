...
# ---------------------------------------------------------------------------
# Prompt templates (loaded from app/prompts/)
# ---------------------------------------------------------------------------

_PROMPT_DIR         = Path(__file__).resolve().parents[1] / "prompts"
_PROMPT             = (_PROMPT_DIR / "market_analyzer_prompt.txt").read_text(encoding="utf-8")
_PAGES_WITH_CONTENT = (_PROMPT_DIR / "market_analyzer_pages_with_content.txt").read_text(encoding="utf-8")
_PAGES_NO_CONTENT   = (_PROMPT_DIR / "market_analyzer_pages_no_content.txt").read_text(encoding="utf-8")
...
class MarketAnalyzer:
    ...
    def analyze_all(
        ...
        """
        mc  = pymongo.MongoClient(self.mongo_uri, tlsCAFile=mongo_tls_ca_file(), server_api=ServerApi("1"), connect=False, serverSelectionTimeoutMS=5000)
    ...
    def load_from_mongo(mongo_uri: str, mongo_db: str = "kairo") -> Optional[dict]:
        ...
        try:
            mc  = pymongo.MongoClient(mongo_uri, tlsCAFile=mongo_tls_ca_file(), server_api=ServerApi("1"), connect=False, serverSelectionTimeoutMS=5000)

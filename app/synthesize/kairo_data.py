    ...
    try:
        ...
        for r in top_bridges:
            ...
            facts["bridge_flows"].append({
                ...
                "window_end":    r.get("time_bucket") or r.get("ingested_at") or "",
            })
    except Exception as exc:
...
def _bucket_episodes(episodes: list[dict], total_days: int) -> list[dict]:
    """
    Return up to 3 episodes, always pinning Day 0 (first detection) at the top,
    followed by the 2 most recent episodes.
    """
    if not episodes or len(episodes) <= 3:
        return episodes

    first = episodes[0]   # Day 0 — always pinned
    rest  = episodes[1:]

    # Pin Day 0 + the 2 most recent episodes (deduplicated)
    recent_two = rest[-2:]
    return [first] + recent_two
